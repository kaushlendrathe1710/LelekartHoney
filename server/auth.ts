...
import { generateOTP, saveOTP, verifyOTP, sendOTPEmail } from "./helpers/email";
import { z } from "zod";
...
export function setupAuth(app: Express) {
      ...
      try {
        ...
        if (!isSpecialAdmin(email)) {
          return res.status(403).json({
            error:
              "This login method is restricted to authorized administrators only",
          });
        }
  ...
  passport.deserializeUser(async (serialized: any, done) => {
    try {
      ...
      if (
        serialized.isImpersonating === true &&
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (req.user.role !== "admin")
          return res
            .status(403)
            .json({ error: "Only administrators can impersonate users" });

        // Don't allow nested impersonation
        if (req.user.isImpersonating) {
          return res.status(400).json({
            error:
              "You are already impersonating a user. Please end the current impersonation first.",
          });
        }
        ...
        if (req.user.id === targetUserId) {
          return res
            .status(400)
            .json({ error: "You cannot impersonate yourself" });
        }
        ...
        console.log(
          `Admin ${req.user.username} (ID: ${req.user.id}) started impersonating user ${targetUser.username} (ID: ${targetUser.id})`
        );
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (!req.user.isImpersonating || !req.user.originalUserId) {
          return res
            .status(400)
            .json({ error: "You are not currently impersonating anyone" });
        }
        ...
        console.log(
          `Ending impersonation: User ${req.user.username} (ID: ${req.user.id}) returning to original user ${originalUser.username} (ID: ${originalUser.id})`
        );
